...
void reopen_logs_ext(int slave_mode) {
  ...
  if (logname &&
      (fd = open(logname, O_WRONLY | O_APPEND | O_CREAT, 0640)) != -1) {
    dup2(fd, 1);
    dup2(fd, 2);
    if (fd > 2) {
      close(fd);
...
void reopen_logs(void) { reopen_logs_ext(0); }
...
void kdb_write(int fd, const void *buf, long long count, const char *filename) {
  ...
  while (count) {
    long long l = !reindex_speed       ? count
                  : count >= (1 << 20) ? (1 << 20)
                                       : count;
    ...
    if (w <= 0) {
      assert(-1 <= w);
      if (write_fail_count < 10000 &&
          (w == 0 || errno == EINTR || errno == EAGAIN ||
           errno == EWOULDBLOCK)) {
        write_fail_count++;
        continue;
      }
      ...
      fprintf(stderr,
              "kdb_write: write %lld bytes to the file '%s' returns %lld. %m\n",
              l, filename, w);
      exit(1);
    }
    assert(w <= l);
...
static inline void kwrite_print_int(char **s, const char *name, int name_len,
                                    int i) {
  if (i < 0) {
    i = INT_MAX;
  }

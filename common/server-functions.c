...
int raise_file_rlimit(int maxfiles) {
  ...
  if (getrlimit(RLIMIT_NOFILE, &rlim) != 0) {
    ...
    if (setrlimit(RLIMIT_NOFILE, &rlim) != 0) {
      kprintf("failed to set rlimit for open files. Try running as root or "
...
void parse_option_down(struct engine_parse_option *P) {
  struct engine_parse_option *Q = P + 1;
  while (Q < engine_parse_options + engine_parse_options_num &&
         Q->smallest_val < P->smallest_val) {
    Q++;
  }
  Q--;
  if (Q != P) {
...
void parse_option_internal(const char *name, int arg,
  ...
  if (engine_parse_options_num == engine_parse_options_size) {
    ...
    engine_parse_options =
        realloc(engine_parse_options,
                sizeof(struct engine_parse_option) * engine_parse_options_size);
  }
  assert(engine_parse_options_num < engine_parse_options_size);
  struct engine_parse_option *P =
      &engine_parse_options[engine_parse_options_num++];
  P->arg = arg;
  P->flags = flags;
...
void parse_option_builtin(const char *name, int arg, int *var, int val,
...
int parse_engine_options_long(int argc, char **argv) {
  ...
  struct option *longopts =
      malloc((total_longopts + 1) * sizeof(struct option));
  int lpos = 0;
  int spos = 0;
...
void add_builtin_parse_options(void) {
  parse_option_builtin("verbosity", optional_argument, 0, 'v',
                       LONGOPT_COMMON_SET, "sets or increases verbosity level");
  parse_option_builtin("help", no_argument, 0, 'h', LONGOPT_COMMON_SET,
                       "prints help and exits");
  parse_option_builtin("user", required_argument, 0, 'u', LONGOPT_COMMON_SET,
                       "sets user name to make setuid");
  parse_option_builtin("log", required_argument, 0, 'l', LONGOPT_COMMON_SET,
                       "sets log file name");
  parse_option_builtin("daemonize", optional_argument, 0, 'd',
                       LONGOPT_COMMON_SET,
                       "changes between daemonize/not daemonize mode");
  parse_option_builtin("nice", required_argument, 0, 202, LONGOPT_COMMON_SET,
                       "sets niceness");
  parse_option_ex("msg-buffers-size", required_argument, 0, 208,
                  LONGOPT_COMMON_SET, builtin_parse_option,
                  "sets maximal buffers size (default %lld)",
                  (long long)MSG_DEFAULT_MAX_ALLOCATED_BYTES);
  // parse_option_builtin ("tl-history", optional_argument, 0, 210,
  // LONGOPT_NET_SET, "long }, parse_option_builtin ("tl-op-stat", no_argument,
  // 0, 211, LONGOPT_NET_SET, "enabled stat about op usage"); {

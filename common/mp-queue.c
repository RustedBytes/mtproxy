...
static long mpq_push_c_impl(struct mp_queue *MQ, mqn_value_t val, int flags);
...
struct mp_queue_block *alloc_mpq_block(mqn_value_t first_val,
  ...
  if (allow_recursion) {
    QB = mpq_pop_c_impl(is_small ? &MqGarbageSmallBlocks : &MqGarbageBlocks,
                        MPQF_RECURSIVE);
    if (QB) {
      if (!is_hazard_ptr(QB, 0, 2)) {
        ...
        mpq_push_c_impl(is_small ? &MqGarbageSmallBlocks : &MqGarbageBlocks, QB,
                        MPQF_RECURSIVE);
        QB = 0;
      }
    }
    if (!QB) {
      QB = mpq_pop_c_impl(is_small ? &MqPreparedSmallBlocks : &MqPreparedBlocks,
                          MPQF_RECURSIVE);
      if (QB) {
  ...
  if (!QB) {
    char *new_block = malloc(offsetof(struct mp_queue_block, mqb_nodes) +
                             size * (2 * sizeof(void *)) + MPQ_BLOCK_ALIGNMENT -
                             sizeof(void *));
    assert(new_block);
    ...
    if (is_small) {
      ...
      if (t >= mpq_small_blocks_allocated_max) {
        __sync_bool_compare_and_swap(&mpq_small_blocks_allocated_max,
                                     mpq_small_blocks_allocated_max, t + 1);
      }
    } else {
...
void free_mpq_block(struct mp_queue_block *QB) {
  ...
  assert((unsigned)QB->mqb_align_bytes < MPQ_BLOCK_ALIGNMENT &&
         !(QB->mqb_align_bytes & (sizeof(void *) - 1)));
...
mqn_value_t mpq_block_pop(struct mp_queue_block *QB) {
  ...
  while (1) {
    ...
    while (1) {
      ...
      /* somebody changed this element while we were inspecting it, make another
       * loop iteration */
    }
    barrier();
...
static mqn_value_t mpq_pop_c_impl(struct mp_queue *MQ, int flags) {
  ...
  while (1) {
    ...
    if (__sync_bool_compare_and_swap(&MQ->mq_head, QB, QB->mqb_next)) {
      // want to free QB here, but this is complicated if somebody else holds a
      // pointer
      if (is_hazard_ptr(QB, 0, 2) <= 1) {
        ...
        mpq_push_c_impl(QB->mqb_size == MPQ_SMALL_BLOCK_SIZE
                            ? &MqGarbageSmallBlocks
                            : &MqGarbageBlocks,
                        QB, flags & MPQF_RECURSIVE);
      }
    }
  }
...
#define DBG(c) // fprintf (stderr, "[%d] pushing %lx to %p,%p: %c\n",
    ...
    if (__sync_fetch_and_add (&QB->mqb_next_allocators, 1)) {
      // somebody else will allocate next block; busy wait instead of spuruous
    alloc/free DBG('B') while (!QB->mqb_next) { barrier ();
      }
      DBG('C')
      continue;
    }
    */
    ...
    if (__sync_bool_compare_and_swap(&QB->mqb_next, 0, NQB)) {
      ...
      mpq_push_c_impl(is_small ? &MqPreparedSmallBlocks : &MqPreparedBlocks,
                      NQB, 0);
      __sync_fetch_and_add(&mpq_blocks_prepared, 1);
    }
  }

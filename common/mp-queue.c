...
struct mp_queue_block *alloc_mpq_block (mqn_value_t first_val, int allow_recursion, int is_small) {
  ...
  if (allow_recursion) {
    QB = mpq_pop_c_impl (is_small ? &MqGarbageSmallBlocks : &MqGarbageBlocks, MPQF_RECURSIVE);
    if (QB) {
	...
	mpq_push_c_impl (is_small ? &MqGarbageSmallBlocks : &MqGarbageBlocks, QB, MPQF_RECURSIVE);
	QB = 0;
      }
    }
    if (!QB) {
      QB = mpq_pop_c_impl (is_small ? &MqPreparedSmallBlocks : &MqPreparedBlocks, MPQF_RECURSIVE);
      if (QB) {
...
static mqn_value_t mpq_pop_c_impl (struct mp_queue *MQ, int flags) {
  ...
  while (1) {
	...
	mpq_push_c_impl (QB->mqb_size == MPQ_SMALL_BLOCK_SIZE ? &MqGarbageSmallBlocks : &MqGarbageBlocks, QB, flags & MPQF_RECURSIVE);
      }
    }
  }
...
static int mpq_is_empty_c_impl (struct mp_queue *MQ) {
  ...
  while (1) {
    ...
    *hptr = QB;
	...
	// ... put QB into some GC queue? ...
	QB->mqb_magic = MQ_BLOCK_GARBAGE_MAGIC;
	mpq_push_c_impl (QB->mqb_size == MPQ_SMALL_BLOCK_SIZE ? &MqGarbageSmallBlocks : &MqGarbageBlocks, QB, 0);
      }
    }
  }
  *hptr = 0;
...
#define DBG(c) // fprintf (stderr, "[%d] pushing %lx to %p,%p: %c\n", mpq_this_thread_id, (long) val, MQ, QB, c);
    ...
    */
	...
	hptr[2] = NQB;
      ...
      mpq_push_c_impl (is_small ? &MqPreparedSmallBlocks : &MqPreparedBlocks, NQB, 0);
      __sync_fetch_and_add (&mpq_blocks_prepared, 1);
    }
  }
#undef DBG

...
void init_server_PID(unsigned ip, int port) {
  int rc =
      mtproxy_ffi_pid_init_server((mtproxy_ffi_process_id_t *)&PID, ip, port);
  assert(rc == 0);
};

/* returns 1 if X is a special case of Y, 2 if they match completely */
int matches_pid(npid_t *X, npid_t *Y) {
  int rc = mtproxy_ffi_matches_pid((const mtproxy_ffi_process_id_t *)X,
                                   (const mtproxy_ffi_process_id_t *)Y);
  assert(rc >= 0 && rc <= 2);
  return rc;
}
...
int process_id_is_newer(struct process_id *a, struct process_id *b) {
  int rc = mtproxy_ffi_process_id_is_newer((const mtproxy_ffi_process_id_t *)a,
                                           (const mtproxy_ffi_process_id_t *)b);
  assert(rc == 0 || rc == 1);
  return rc;
}

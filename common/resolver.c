...
static int pr[] = {29,      41,      59,       89,       131,      197,
                   293,     439,     659,      991,      1481,     2221,
                   3329,    4993,    7487,     11239,    16843,    25253,
                   37879,   56821,   85223,    127837,   191773,   287629,
                   431441,  647161,  970747,   1456121,  2184179,  3276253,
                   4914373, 7371571, 11057357, 16586039, 24879017, 37318507};
...
static struct host *getHash(struct resolver_conf *R, const char *name, int len,
                            unsigned ip) {
  int h1 = 0, h2 = 0, i;
...
static int kdb_load_hosts_internal(void) {
  ...
  if (Hosts.hosts_loaded > 0 && Hosts.fsize == s.st_size &&
      Hosts.ftime == s.st_mtime) {
    return 0;
  }
...
int parse_ipv6(unsigned short ipv6[8], char *str) { return -1; }
...
char *detect_hostname(void) {
  ...
  i = 0;
  while ((hostname[i] >= '0' && hostname[i] <= '9') || hostname[i] == '.' ||
         hostname[i] == '-' || hostname[i] == '_' ||

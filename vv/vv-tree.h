...
#define DEFINE_HASH(prefix, name, value_t, value_compare, value_hash)          \
  prefix hash_elem_##name##_t *hash_lookup_##name(                             \
      hash_table_##name##_t *T, value_t x) __attribute__((unused));            \
  ...
  prefix void hash_clear_##name(hash_table_##name##_t *T)                      \
      __attribute__((unused));                                                 \
  prefix void hash_clear_act_##name(                                           \
      hash_table_##name##_t *T, void (*act)(value_t)) __attribute__((unused)); \
  prefix hash_elem_##name##_t *hash_lookup_##name(hash_table_##name##_t *T,    \
                                                  value_t x) {                 \
    ...
    for (i = 0; i < T->size; i++) {                                            \
      if (T->E[i]) {                                                           \
        ...
        T->E[i] = nullptr;                                                     \
                                                                               ...
                                                                               \
  prefix void hash_clear_act_##name(hash_table_##name##_t *T,                  \
                                    void (*act)(value_t)) {                    \
    int i;                                                                     \
    for (i = 0; i < T->size; i++) {                                            \
      if (T->E[i]) {                                                           \
...
#define DEFINE_HASH_STD_ALLOC_PREFIX(prefix, name, value_t, value_compare,     \
                                     value_hash)                               \
  ...
  void hash_free_##name(hash_elem_##name##_t *E) { zfree(E, sizeof(*E)); }

#define DEFINE_HASH_STDNOZ_ALLOC_PREFIX(prefix, name, value_t, value_compare,  \
                                        value_hash)                            \
  DECLARE_HASH_TYPE(name, value_t)                                             \
  prefix hash_elem_##name##_t *hash_alloc_##name(value_t x);                   \
  prefix void hash_free_##name(hash_elem_##name##_t *T);                       \
  ...
  void hash_free_##name(hash_elem_##name##_t *E) { free(E); }
...
#define DEFINE_HASH_STDNOZ_ALLOC(name, value_t, value_compare, value_hash)     \
  DEFINE_HASH_STDNOZ_ALLOC_PREFIX(static, name, value_t, value_compare,        \
                                  value_hash)
...
#define HASH_DEG2(name, deg)                                                   \
  ...
  static struct hash_table_##name hash_table_##name##_ptr = {                  \
      .E = _hash_arr_##name, .size = (1 << (deg)), .mask = (1 << (deg)) - 1};  \

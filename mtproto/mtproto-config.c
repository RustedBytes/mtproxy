...
// flags = 0 -- syntax check only (first pass), flags = 1 -- create targets and
// points as well (second pass) flags: +2 = allow proxies, +4 = allow proxies
// only, +16 = do not load file
int parse_config(struct mf_config *MC, int flags, int config_fd) {
  ...
  int32_t pass_rc = mtproxy_ffi_mtproto_cfg_parse_full_pass(
      parse_start, parse_len, default_cfg_min_connections,
      default_cfg_max_connections, (flags & 1) ? 1 : 0,
      (uint32_t)MAX_CFG_CLUSTERS, (uint32_t)MAX_CFG_TARGETS, actions,
      (uint32_t)MAX_CFG_TARGETS, &parsed);
  if (pass_rc != MTPROXY_FFI_MTPROTO_CFG_PARSE_FULL_PASS_OK) {
    ...
    case MTPROXY_FFI_MTPROTO_CFG_PARSE_FULL_PASS_ERR_MISSING_PROXY_DIRECTIVES:
      syntax("expected to find a mtproto-proxy configuration with `proxy' "
             "directives");
      break;
  ...
  for (uint32_t i = 0; i < parsed.actions_len; i++) {
    ...
    if (A->step.target_index >= (uint32_t)MAX_CFG_TARGETS ||
        A->step.target_index >= parsed.tot_targets) {
    ...
    if (flags & 1) {
      ...
      vkprintf(3, "new target %p created (%d): ip %s, port %d\n", D,
               was_created, inet_ntoa(default_cfg_ct.target),
               default_cfg_ct.port);
    }

    if (A->step.cluster_index < 0 ||
        A->step.cluster_index >= MAX_CFG_CLUSTERS) {
    ...
    case MTPROXY_FFI_MTPROTO_CFG_CLUSTER_TARGETS_ACTION_SET_TARGET:
      ...
      if (A->step.cluster_targets_index >= (uint32_t)MAX_CFG_TARGETS ||
          A->step.cluster_targets_index >= A->step.tot_targets_after) {
    ...
    if (A->step.cluster_decision_kind ==
        MTPROXY_FFI_MTPROTO_CFG_CLUSTER_APPLY_DECISION_KIND_CREATE_NEW) {
      vkprintf(3, "-> added target to new auth_cluster #%d\n",
               A->step.cluster_index);
    } else if (
        A->step.cluster_decision_kind ==
        MTPROXY_FFI_MTPROTO_CFG_CLUSTER_APPLY_DECISION_KIND_APPEND_LAST) {
  ...
  if (parsed.has_default_cluster_index) {
    if (parsed.default_cluster_index >= parsed.auth_clusters ||
        parsed.default_cluster_index >= (uint32_t)MAX_CFG_CLUSTERS) {
...
// flags: +1 = create targets and connections, +2 = allow proxies, +4 = allow
// proxies only, +16 = do not re-load file itself, +32 = preload config +
// perform syntax check, do not apply
int do_reload_config(int flags) {
  ...
  if (res < 0) {
    kprintf("error while re-reading config file %s, new configuration NOT "
            "applied\n",
            config_filename);
    return res;
  }
  ...
  kprintf("configuration file %s re-read successfully (%d bytes parsed), new "

...
15. Complete Remaining C-to-Rust Port in `rust/mtproxy-bin`
   ...
   - [x] 2026-02-11: Ported remaining directive-loop cluster selection/finalization semantics from `mtproto/mtproto-config.c` into `runtime::mtproto::config` (`mf_cluster_lookup_index`, `finalize_parse_config_state`), exposed them via FFI (`mtproxy_ffi_mtproto_cfg_lookup_cluster_index`, `mtproxy_ffi_mtproto_cfg_finalize`), and wired C parse flow to Rust lookup/finalize helpers while preserving C host-resolution and target-creation behavior.
   - [x] 2026-02-11: Switched runtime `mf_cluster_lookup` in `mtproto/mtproto-config.c` from C linear scan to Rust-backed FFI cluster-index resolution (`mtproxy_ffi_mtproto_cfg_lookup_cluster_index`) while preserving force/default-cluster fallback semantics for live proxy target selection.
   - [x] 2026-02-11: Ported `preinit_config` state-reset call path from `mtproto/mtproto-config.c` to Rust FFI (`mtproxy_ffi_mtproto_cfg_preinit`) backed by `runtime::mtproto::config::preinit_config_snapshot`, and removed the C `preinit_config` implementation from active runtime.
   - [x] 2026-02-11: Collapsed more `parse_config` cluster/apply control flow into Rust helpers by adding Rust-side cluster-decision semantics (`decide_proxy_cluster_apply`) exposed via FFI (`mtproxy_ffi_mtproto_cfg_decide_cluster_apply`) and rewiring C `proxy`/`proxy_for` branches to consume Rust action results (create-new vs append-last vs intermixed/limit errors).

...
/* INVARIANTS FOR MULTITHREAD USE:
  ...
  - msg buffer can be modified if it's reference counter is 1 and overlying msg
  part can be modified
  - msg parts can not have loops
*/

/*
  msg_part mp can be expanded to left, if mp->refcnt=1, mp->part->refcnt=1,
  mp=raw->first, where raw is raw_message we have in this thread msg_part mp can
  be expanded to right, if mp->part->refcnt=1, mp->next=NULL and ((mp is locked)
  or (refcnt on chain from raw->first to mp is 1))
...
struct raw_message {
  struct msg_part *first,
      *last;        // 'last' doesn't increase refcnt of pointed msg_part
...
/* NB: struct raw_message itself is never allocated or freed by the following
   functions since it is usually part (field) of a larger structure
*/
...
int rwm_encrypt_decrypt_to(struct raw_message *raw, struct raw_message *res,
                           int bytes, mtproxy_aesni_ctx_t *ctx, int block_size);

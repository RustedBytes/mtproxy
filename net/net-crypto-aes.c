...
MODULE_STAT_TYPE { int allocated_aes_crypto, allocated_aes_crypto_temp; };
...
extern int32_t mtproxy_ffi_crypto_aes_create_keys(
    void *out, int32_t am_client, const uint8_t nonce_server[16],
    const uint8_t nonce_client[16], int32_t client_timestamp,
    uint32_t server_ip, uint16_t server_port, const uint8_t server_ipv6[16],
    uint32_t client_ip, uint16_t client_port, const uint8_t client_ipv6[16],
    const uint8_t *secret, int32_t secret_len, const uint8_t *temp_key,
...
int aes_crypto_init(connection_job_t c, void *key_data, int key_data_len) {
  ...
  T->read_aeskey = evp_cipher_ctx_init_kind(EVP_CIPHER_KIND_AES_256_CBC,
                                            D->read_key, D->read_iv, 0);
  T->write_aeskey = evp_cipher_ctx_init_kind(EVP_CIPHER_KIND_AES_256_CBC,
                                             D->write_key, D->write_iv, 1);
...
int aes_crypto_ctr128_init(connection_job_t c, void *key_data,
  ...
  T->read_aeskey =
      evp_cipher_ctx_init_kind(EVP_CIPHER_KIND_AES_256_CTR, D->read_key,
                               D->read_iv, 1); // NB: is_encrypt == 1 here!
  T->write_aeskey = evp_cipher_ctx_init_kind(EVP_CIPHER_KIND_AES_256_CTR,
                                             D->write_key, D->write_iv, 1);
  CONN_INFO(c)->crypto = T;
  return 0;
}
...
int aes_load_pwd_file(const char *filename) {
  ...
  *(long *)rand_buf ^= lrand48_j();
  ...
  if (r < MIN_PWD_LEN || r > MAX_PWD_LEN) {
    vkprintf(1,
             "secret file %s too long or too short: loaded %d bytes, expected "
             "%d..%d\n",
...
int aes_create_keys(struct aes_key_data *R, int am_client,
  ...
  int32_t rc = mtproxy_ffi_crypto_aes_create_keys(
      R, am_client, (const uint8_t *)nonce_server,

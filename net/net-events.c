...
static event_t *pop_heap_head(void) {
  ...
  while (1) {
    ...
    if (j > N)
      break;
    if (j < N && greater_ev(ev_heap[j], ev_heap[j + 1]) > 0)
      j++;
    y = ev_heap[j];
    if (greater_ev(x, y) <= 0)
      break;
...
int epoll_unconv_flags(int f) { return mtproxy_ffi_net_epoll_unconv_flags(f); }
...
int epoll_insert(int fd, int flags) {
  ...
  ev->ready = 0; // !!! this bugfix led to some AIO-related bugs, now fixed with
                 // the aid of C_REPARSE flag
  if ((ev->state & (EVT_LEVEL | EVT_RWX | EVT_IN_EPOLL)) ==
      flags + EVT_IN_EPOLL) {
    return 0;
  }
  ev->state =
      (ev->state & ~(EVT_LEVEL | EVT_RWX)) | (flags & (EVT_LEVEL | EVT_RWX));
  ef = epoll_conv_flags(flags);
  if (ef != ev->epoll_state || (flags & EVT_NEW) ||
    ...
    vkprintf(2, "epoll_mod(%d,0x%08x,%d,%d,%08x)\n", epoll_fd, ev->state, fd,
             ee.data.fd, ee.events);

    if (epoll_ctl(epoll_fd,
                  (ev->state & EVT_IN_EPOLL) ? EPOLL_CTL_MOD : EPOLL_CTL_ADD,
                  fd, &ee) < 0) {
...
int epoll_run_timers(void) { return thread_run_timers(); }
...
int epoll_runqueue(void) {
  ...
  ev_timestamp += 2;
  while (ev_heap_size && (ev = ev_heap[1])->timestamp < ev_timestamp &&
...
int new_socket(int mode, int nonblock) {
  ...
  if ((socket_fd = socket(mode & SM_IPV6 ? AF_INET6 : AF_INET,
                          mode & SM_UDP ? SOCK_DGRAM : SOCK_STREAM, 0)) == -1) {
    perror("socket()");
    return -1;
  }
...
int server_socket(int port, struct in_addr in_addr, int backlog, int mode) {
  ...
  if (mode & SM_UDP) {
    ...
    assert(flags == 1);
    assert(setsockopt(socket_fd, SOL_SOCKET, SO_KEEPALIVE, &flags,
                      sizeof(flags)) >= 0);
    if (enable_often_tcp_keep_alive) {
      int x = 40;
      assert(setsockopt(socket_fd, IPPROTO_TCP, TCP_KEEPIDLE, &x, sizeof(x)) >=
             0);
      assert(setsockopt(socket_fd, IPPROTO_TCP, TCP_KEEPINTVL, &x, sizeof(x)) >=
             0);
      x = 5;
      assert(setsockopt(socket_fd, IPPROTO_TCP, TCP_KEEPCNT, &x, sizeof(x)) >=
             0);
    }
  }
...
unsigned get_my_ipv4(void) {
  ...
  for (ifa = ifa_first; ifa; ifa = ifa->ifa_next) {
    ...
    if ((ip & (-1 << 24)) == (10 << 24) &&
        (mask < my_netmask || (my_ip >> 24) != 10)) {
      my_ip = ip;
      my_netmask = mask;
      my_iface = ifa->ifa_name;
    } else if ((ip & (-1 << 24)) != (127 << 24) && mask < my_netmask &&
               (my_ip >> 24) != 10) {
      my_ip = ip;
      my_netmask = mask;
      my_iface = ifa->ifa_name;
  ...
  vkprintf(1, "using main IP %d.%d.%d.%d/%d at interface %s\n", (my_ip >> 24),
           (my_ip >> 16) & 255, (my_ip >> 8) & 255, my_ip & 255,
...
const char *conv_addr(in_addr_t a, char *buf) {
  ...
  sprintf(buf, "%d.%d.%d.%d", a & 255, (a >> 8) & 255, (a >> 16) & 255,
          a >> 24);
  return buf;
}
...
int conv_ipv6_internal(const unsigned short a[8], char *buf) {
  ...
  if (l == 5 && a[5] == 0xffff) {
    return sprintf(buf, "::ffff:%d.%d.%d.%d", a[6] & 255, a[6] >> 8, a[7] & 255,
                   a[7] >> 8);
  }
  char *ptr = buf;
  if (l) {
...
const char *show_ip(unsigned ip) {
  ...
  res = ptr;
  ptr += sprintf(ptr, "%d.%d.%d.%d", ip >> 24, (ip >> 16) & 0xff,
                 (ip >> 8) & 0xff, ip & 0xff) +
         1;
  return res;
}

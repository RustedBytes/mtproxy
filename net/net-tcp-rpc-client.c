...
extern int32_t
mtproxy_ffi_tcp_rpc_client_packet_len_state(int32_t packet_len,
...
extern int32_t
mtproxy_ffi_tcp_rpc_client_process_nonce_packet(const uint8_t *packet, int32_t packet_len,
                                               int32_t allow_unencrypted, int32_t allow_encrypted,
                                               int32_t require_dh, int32_t has_crypto_temp,
                                               int32_t nonce_time, int32_t main_secret_len,
...
static inline int tcp_rpc_client_packet_len_state(int packet_len,
                                                  int max_packet_len) {
  int32_t state =
      mtproxy_ffi_tcp_rpc_client_packet_len_state(packet_len, max_packet_len);
...
static int tcp_rpcc_process_nonce_packet(connection_job_t C,
  ...
  if (mtproxy_ffi_tcp_rpc_client_process_nonce_packet(
          (const uint8_t *)&P, packet_len, (D->crypto_flags & RPCF_ALLOW_UNENC),
          (D->crypto_flags & RPCF_ALLOW_ENC), (D->crypto_flags & RPCF_REQ_DH),
          c->crypto_temp ? 1 : 0, D->nonce_time, main_secret.secret_len,
  ...
  case RPC_CRYPTO_AES:
    ...
    if (c->crypto_temp) {
      if (((struct crypto_temp_dh_params *)c->crypto_temp)->magic ==
          CRYPTO_TEMP_DH_PARAMS_MAGIC) {
...
static int tcp_rpcc_process_handshake_packet(connection_job_t C,
  ...
  if (!matches_pid(&P.sender_pid, &D->remote_pid) &&
    ...
    vkprintf(1,
             ...
             "remote %08x:%d:%d:%d\n",
             D->remote_pid.ip, D->remote_pid.port, D->remote_pid.pid,
             D->remote_pid.utime, P.sender_pid.ip, P.sender_pid.port,
             P.sender_pid.pid, P.sender_pid.utime);
...
int tcp_rpcc_parse_execute(connection_job_t C) {
  ...
  while (1) {
    ...
    unsigned packet_crc32 =
        rwm_custom_crc32(&msg, packet_len - 4, D->custom_crc_partial);
    ...
    if (packet_num != D->in_packet_num) {
      vkprintf(1,
               "error while parsing packet: got packet num %d, expected %d\n",
...
void tcp_force_enable_dh(void) { force_rpc_dh |= 4; }
...
int tcp_rpcc_init_crypto(connection_job_t C) {
  ...
  if (D->crypto_flags & RPCF_REQ_DH) {
    ...
    struct tcp_rpc_nonce_dh_packet *dh =
        (struct tcp_rpc_nonce_dh_packet *)((char *)&buf +
                                           4 * (extra_keys -
                                                RPC_MAX_EXTRA_KEYS));
    ...
    len = offsetof(struct tcp_rpc_nonce_ext_packet, extra_key_select) +
          4 * extra_keys;
  }
...
int tcp_rpcc_start_crypto(connection_job_t C, char *nonce, int key_select,
  ...
  if (aes_create_keys(&aes_keys, 1, nonce, D->nonce, D->nonce_time,
                      nat_translate_ip(c->remote_ip), c->remote_port,
                      c->remote_ipv6, nat_translate_ip(c->our_ip), c->our_port,
                      c->our_ipv6, secret, temp_key, temp_key_len) < 0) {
    return -1;
  }

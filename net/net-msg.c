...
static inline struct msg_part *alloc_msg_part(void) {
  ...
  struct msg_part *mp = (struct msg_part *)malloc(sizeof(struct msg_part));
...
struct msg_part *rwm_lock_last_part(struct raw_message *raw) /* {{{ */ {
  ...
  if (mp->next || raw->last_offset != mp->data_end) {
    ...
    // trying to append bytes to a sub-message of a longer chain, have to fork
    // the chain
    fork_message_chain(raw);
  } else {
    if (mp->magic != MSG_PART_MAGIC ||
        !__sync_bool_compare_and_swap(&mp->magic, MSG_PART_MAGIC,
                                      MSG_PART_LOCKED_MAGIC)) {
      fork_message_chain(raw);
    } else {
      locked = mp;
      barrier();
...
int rwm_push_data_ext(struct raw_message *raw, const void *data,
  ...
  if (!raw->first) {
    ...
    // no need to lock in this case, because refcnt in chain is 1 in
    // newly-created message
    struct msg_buffer *X = alloc_msg_buffer(
        0, alloc_bytes >= small_buffer - prepend ? std_buffer : small_buffer);
    if (!X) {
      return 0;
    }
    mp = new_msg_part(0, X);
  ...
  while (alloc_bytes > 0) {
    mpl = mp;
    struct msg_buffer *X = alloc_msg_buffer(
        mpl->part, raw->total_bytes + alloc_bytes >= std_buffer ? std_buffer
                                                                : small_buffer);
    if (!X) {
      break;
    }
...
int rwm_push_data_front(struct raw_message *raw, const void *data,
  ...
  while (alloc_bytes) {
    struct msg_buffer *X = alloc_msg_buffer(
        raw->first ? raw->first->part : 0,
        alloc_bytes >= MSG_SMALL_BUFFER ? MSG_STD_BUFFER : MSG_SMALL_BUFFER);
    assert(X);
...
void *rwm_prepend_alloc(struct raw_message *raw, int alloc_bytes) /* {{{ */ {
  ...
  if (raw->first->refcnt == 1 && raw->first->offset >= alloc_bytes &&
  ...
  assert(raw->first_offset == raw->first->offset);
  struct msg_buffer *X =
      alloc_msg_buffer(raw->first ? raw->first->part : 0, alloc_bytes);
  assert(X);
  int size = X->chunk->buffer_size;
...
int rwm_split(struct raw_message *raw, struct raw_message *tail,
  ...
  while (bytes) {
    assert(mp);
    int sz = (mp == raw->last ? raw->last_offset : mp->data_end) -
             (mp == raw->first ? raw->first_offset : mp->offset);
    if (mp->refcnt != 1) {
      ok = 0;
    }
    if (sz < bytes) {
      ...
      raw->last = mp;
      raw->last_offset =
          (mp == raw->first ? raw->first_offset : mp->offset) + bytes;
      tail->first = mp;
      tail->first_offset = raw->last_offset;
...
int rwm_union(struct raw_message *raw, struct raw_message *tail) /* {{{ */ {
  ...
  if (!raw->last) {
    *raw = *tail;
    ...
    // if there would be loop, that last msg_part in chains of raw and tail are
    // same then they can not be simultaneously locked, so this call will make
    // copy of chain
...
int rwm_dump_sizes(struct raw_message *raw) /* {{{ */ {
  ...
  if (!raw->first) {
    ...
    fprintf(stderr, "(");
    while (mp != 0) {
      int size = (mp == raw->last ? raw->last_offset : mp->data_end) -
                 (mp == raw->first ? raw->first_offset : mp->offset);
...
int rwm_process_ex(struct raw_message *raw, int bytes, int offset, int flags,
  ...
  // correct, because if raw->last == raw->first all bytes garanteed to be in
  // this (only) msg part
  if (raw->total_bytes - offset <= raw->last_offset - raw->last->offset) {
    int x = raw->total_bytes - offset;
    int r = process_block(extra, raw->last->part->data + raw->last_offset - x,
                          bytes);
  ...
  while (mp) {
    ...
    int len =
        (mp == raw->last) ? raw->last_offset - start : mp->data_end - start;
    ...
    if (len >= offset) {
      ...
      while (bytes) {
        if (len >= bytes) {
          r = bytes > 0 ? process_block(extra, mp->part->data + start, bytes)
                        : 0;
...
int rwm_sha1(struct raw_message *raw, int bytes, unsigned char output[20]) {
  ...
  struct sha1_copy_state S = {.buf = tmp, .offset = 0};
...
int rwm_get_block_ptr_bytes(struct raw_message *raw) {
  ...
  while (1) {
    assert(mp);
    int bytes = ((mp == raw->last) ? raw->last_offset : mp->data_end) -
                raw->first_offset;
    if (bytes) {
      return bytes;
    }
...
int rwm_process_encrypt_decrypt(struct rwm_encrypt_decrypt_tmp *x,
                                const void *data, int len) {
  int bsize = x->block_size;
  struct raw_message *res = x->raw;
  if (!x->buf_left) {
    struct msg_buffer *X = alloc_msg_buffer(
        res->last->part, x->left >= MSG_STD_BUFFER ? MSG_STD_BUFFER : x->left);
    assert(X);
  ...
  if (x->bp) {
    ...
    if (len >= to_fill) {
      ...
      if (x->buf_left >= bsize) {
        ...
        struct msg_buffer *X = alloc_msg_buffer(
            res->last->part, x->left + len + bsize >= MSG_STD_BUFFER
                                 ? MSG_STD_BUFFER
                                 : x->left + len + bsize);
        assert(X);
        struct msg_part *mp = new_msg_part(res->last, X);
  ...
  while (1) {
    if (x->buf_left < bsize) {
      struct msg_buffer *X = alloc_msg_buffer(
          res->last->part,
          x->left + len >= MSG_STD_BUFFER ? MSG_STD_BUFFER : x->left + len);
      assert(X);
      struct msg_part *mp = new_msg_part(res->last, X);
      res->last->next = mp;
      res->last = mp;
    ...
    assert(res->last_offset >= 0);
    assert(x->buf_left >= 0);
    assert(x->buf_left + res->last_offset <=
...
int rwm_encrypt_decrypt_to(struct raw_message *raw, struct raw_message *res,
  ...
  if (!res->last || res->last->part->refcnt != 1) {
    ...
    struct msg_buffer *X =
        alloc_msg_buffer(res->last ? res->last->part : 0,
                         l >= MSG_STD_BUFFER ? MSG_STD_BUFFER : l);
    assert(X);
    struct msg_part *mp = new_msg_part(res->last, X);
    if (res->last) {
      res->last->next = mp;
      ...
      res->last_offset = res->first_offset = mp->offset = mp->data_end =
          RM_PREPEND_RESERVE;
    }
  }
  ...
  int r = rwm_process_and_advance(raw, bytes,
                                  (void *)rwm_process_encrypt_decrypt, &t);
  if (locked) {
    locked->magic = MSG_PART_MAGIC;
  }
  return r;

...
struct tcp_rpc_client_functions {
  void *info;
  void *rpc_extra;
  int (*execute)(connection_job_t c, int op,
                 struct raw_message *raw); /* invoked from parse_execute() */
  int (*check_ready)(
      connection_job_t c); /* invoked from rpc_client_check_ready() */
  int (*flush_packet)(
      connection_job_t c); /* execute this to push query to server */
  int (*rpc_check_perm)(
      connection_job_t c); /* 1 = allow unencrypted, 2 = allow encrypted */
  int (*rpc_init_crypto)(connection_job_t c); /* 1 = ok; -1 = no crypto */
  int (*rpc_start_crypto)(connection_job_t c, char *nonce, int key_select,
                          unsigned char *temp_key,
                          int temp_key_len); /* 1 = ok; -1 = no crypto */
...
#define TCP_RPCC_FUNC(c)                                                       \
  ((struct tcp_rpc_client_functions *)(CONN_INFO(c)->extra))

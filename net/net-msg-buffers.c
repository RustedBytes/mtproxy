...
extern int32_t mtproxy_ffi_msg_buffers_pick_size_index(
...
void init_buffer_chunk_headers(void) {
  ...
  for (i = 0, CH = ChunkHeaders; i < sizeof(default_buffer_sizes) / sizeof(int);
       i++, CH++) {
    CH->magic = MSG_CHUNK_HEAD_MAGIC;
    CH->buffer_size = default_buffer_sizes[i];
    ChunkBufferSizes[i] = CH->buffer_size;
...
static int msg_buffer_pick_size_index(int size_hint) {
  ...
  int32_t rust_si = mtproxy_ffi_msg_buffers_pick_size_index(
      ChunkBufferSizes, buffer_size_values, size_hint);
...
static void lock_chunk_head(struct msg_buffers_chunk *CH) {
  while (1) {
    if (__sync_bool_compare_and_swap(&CH->magic, MSG_CHUNK_HEAD_MAGIC,
...
static int try_lock_chunk(struct msg_buffers_chunk *C) {
  if (C->magic != MSG_CHUNK_USED_MAGIC ||
      !__sync_bool_compare_and_swap(&C->magic, MSG_CHUNK_USED_MAGIC,
...
alloc_new_msg_buffers_chunk(struct msg_buffers_chunk *CH) {
  ...
  int t =
      (MSG_BUFFERS_CHUNK_SIZE - offsetof(struct msg_buffers_chunk, free_cnt)) /
      (buffer_hd_size + 4);
  two_power = 1;
  while (two_power <= t) {
    two_power <<= 1;
  ...
  chunk_buffers =
      (MSG_BUFFERS_CHUNK_SIZE - offsetof(struct msg_buffers_chunk, free_cnt) -
       two_power * 4 - align) /
      buffer_hd_size;
  assert(chunk_buffers > 0 && chunk_buffers < 65536 &&
         chunk_buffers <= two_power);
  ...
  C->first_buffer =
      (struct msg_buffer *)(((long)C +
                             offsetof(struct msg_buffers_chunk, free_cnt) +
                             two_power * 4 + align - 1) &
                            -align);
  assert((char *)(C->first_buffer) + chunk_buffers * buffer_hd_size <=
         (char *)C + MSG_BUFFERS_CHUNK_SIZE);
  ...
  while (1) {
    ...
    __sync_bool_compare_and_swap(&max_allocated_buffer_chunks,
                                 keep_max_allocated_buffer_chunks,
                                 keep_allocated_buffer_chunks);
    if (allocated_buffer_chunks >= max_buffer_chunks - 8 &&
        max_buffer_chunks >= 32 && verbosity < 3) {
      // verbosity = 3;
      // vkprintf (1, "Setting verbosity to 3 (NOTICE) because of high buffer
      // chunk usage (used %d, max %d)\n", allocated_buffer_chunks,
      // max_buffer_chunks);
    }
  }
  prepare_bs_inv(C);
  ...
  memset(&C->free_cnt[two_power + chunk_buffers], 0,
         (two_power - chunk_buffers) * 2);
...
int init_msg_buffers(long max_buffer_bytes) {
  if (!max_buffer_bytes) {
    max_buffer_bytes =
        max_allocated_buffer_bytes ?: MSG_DEFAULT_MAX_ALLOCATED_BYTES;
  }
...
static inline int get_buffer_no(struct msg_buffers_chunk *C,
                                struct msg_buffer *X) {
  unsigned x = ((char *)X - (char *)C->first_buffer);
  x >>= C->bs_shift;
  x *= C->bs_inverse;
  assert(x <= (unsigned)C->tot_buffers &&
         (char *)X == (char *)C->first_buffer + (C->buffer_size + 16) * x);
  return x;
}

struct msg_buffer *alloc_msg_buffer_internal(struct msg_buffer *neighbor,
                                             struct msg_buffers_chunk *CH,
                                             struct msg_buffers_chunk *C_hint,
                                             int si) {
  unsigned magic = CH->magic;
  assert(magic == MSG_CHUNK_HEAD_MAGIC || magic == MSG_CHUNK_HEAD_LOCKED_MAGIC);
  ...
  i -= two_power;
  vkprintf(3,
           "alloc_msg_buffer(%d) [chunk %p, size %d]: tot_buffers = %d, "
           "free_buffers = %d\n",
           i, C, C->buffer_size, CH->tot_buffers, CH->free_buffers);
  assert(i >= 0 && i < C->tot_buffers);

  struct msg_buffer *X = (struct msg_buffer *)((char *)C->first_buffer +
                                               i * (C->buffer_size + 16));
...
struct msg_buffer *alloc_msg_buffer(struct msg_buffer *neighbor,
  ...
  int si = msg_buffer_pick_size_index(size_hint);
  return alloc_msg_buffer_internal(neighbor, &ChunkHeaders[si], ChunkSave[si],
                                   si);
}

int free_std_msg_buffer(struct msg_buffers_chunk *C, struct msg_buffer *X) {
  assert(!X->refcnt && X->magic == MSG_BUFFER_USED_MAGIC &&
         C->magic == MSG_CHUNK_USED_LOCKED_MAGIC && X->chunk == C);
...
static int free_msg_buffer_job(job_t job, int op, struct job_thread *JT) {
  ...
  case JS_RUN: {
    ...
    assert(magic == MSG_CHUNK_USED_MAGIC ||
           magic == MSG_CHUNK_USED_LOCKED_MAGIC);
    C->free_buffer(C, X);
  ...
  if (C->free_buffer == free_std_msg_buffer) {
    ...
    if (!this_job_thread || this_job_thread->thread_class == C->thread_class) {
      ...
      job_t job = create_async_job(
          ...
          C->thread_subclass, sizeof(void *), 0, JOB_REF_NULL);
      *(void **)job->j_custom = X;

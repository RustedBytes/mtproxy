  ...
  if (type == tl_type_raw_msg) {
    ...
    if (d) {
      vkprintf(2, "%s: Good connection #%d for " PID_PRINT_STR "\n", __func__,
               CONN_INFO(d)->fd, PID_TO_PRINT(pid));
      struct tl_out_state *IO = tl_out_state_alloc();
...
long long engine_get_allocated_queries(void) { return queries_allocated; }
...
long long tl_generate_next_qid(int query_type_id) {
  ...
  return ((unsigned long long)((query_type_id << 28) +
                               (lrand48_j() & 0x0fffffff))
          << 32) |
         (++last_qid);
}
...
struct paramed_type *skip_function_any(struct tl_in_state *tlio_in)
    __attribute__((weak));
struct paramed_type *skip_function_any(struct tl_in_state *tlio_in) {
  return NULL;
}
struct paramed_type *fetch_function_any(struct tl_in_state *tlio_in)
    __attribute__((weak));
struct paramed_type *fetch_function_any(struct tl_in_state *tlio_in) {
  return NULL;
}

int skip_type_any(struct tl_in_state *tlio_in, struct paramed_type *P)
    __attribute__((weak));
int skip_type_any(struct tl_in_state *tlio_in, struct paramed_type *P) {
  return -1;
}
int fetch_type_any(struct tl_in_state *tlio_in, struct paramed_type *P)
    __attribute__((weak));
int fetch_type_any(struct tl_in_state *tlio_in, struct paramed_type *P) {
  return -1;
}
...
static inline struct paramed_type *
do_skip_function_any(struct tl_in_state *tlio_in) {
  free_vars_to_be_freed();
  return skip_function_any(tlio_in);
}

static inline struct paramed_type *
do_fetch_function_any(struct tl_in_state *tlio_in) {
  free_vars_to_be_freed();
  tl_printf_clear();
  return fetch_function_any(tlio_in);
}

static inline int do_fetch_type_any(struct tl_in_state *tlio_in,
...
static job_t fetch_query(job_t parent, struct tl_in_state *IO,
  ...
  job_t job = create_async_job(
      ...
      status | JSC_ALLOW(JC_ENGINE, JS_RUN) | JSC_ALLOW(JC_ENGINE, JS_ABORT) |
          JSC_ALLOW(JC_ENGINE, JS_FINISH),
      extra->subclass, sizeof(void *), 0, JOB_REF_CREATE_PASS_N(parent));

  *(void **)job->j_custom = extra;

  if (all_list) {
    insert_job_into_job_list(all_list, JOB_REF_CREATE_PASS(job),
                             JSP_PARENT_ERROR);
  }
...
int query_job_run(job_t job, int fd, int generation) /* {{{ */ {
  ...
  if (op != RPC_INVOKE_REQ) {
    ...
    if (tlf_error(IO)) {
      ...
      struct raw_message r;
      rwm_clone(&r, (struct raw_message *)IO->in);
      res = create_query_job(job, &r, h, tl_aio_timeout, &q->src_pid,
                             q->src_type, fd, generation);
      rwm_free(&r);
    }
  }
  if (h) {
...
int do_create_query_job(struct raw_message *raw, int type,
  ...
  job_t job = create_async_job(
      ...
      JSP_PARENT_RWE | JSC_ALLOW(JC_ENGINE, JS_RUN) |
          ...
          JSC_ALLOW(JC_ENGINE, JS_FINISH),
      -2, sizeof(struct query_work_params), JT_HAVE_TIMER, JOB_REF_NULL);
  ...
  for (i = 0; s[i]; i++) {
    if (s[i] == '\n') {
      if (value_start - key_start > 1 && value_start < i) {
        tl_store_string(s + key_start,
                        value_start - key_start - 1); /* - 1 (trim tabular) */

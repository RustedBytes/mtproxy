...
void engine_add_net_parse_options(void) {
  ...
  parse_option_net_builtin("connections", required_argument, 0, 'c',
                           LONGOPT_TCP_SET, "sets maximal connections number");
  parse_option_net_builtin(
      "port", required_argument, 0, 'p', LONGOPT_NET_SET,
      "<port> or <sport>:<eport> sets listening port number or port range");
  parse_option_net_builtin("aes-pwd", required_argument, 0, 200,
                           LONGOPT_NET_SET, "sets custom secret.conf file");
  parse_option_net_builtin("ipv6", no_argument, 0, '6', LONGOPT_NET_SET,
                           "enables ipv6 TCP/UDP support");
  parse_option_net_builtin("disable-tcp", no_argument, 0, 214, LONGOPT_TCP_SET,
                           "do not open listening tcp socket");
  parse_option_net_builtin("crc32c", no_argument, 0, 224, LONGOPT_TCP_SET,
                           "Try to use crc32c instead of crc32 in tcp rpc");
  parse_option_net_builtin("allow-skip-dh", no_argument, 0, 229,
                           LONGOPT_TCP_SET,
                           "Allow skipping DH during RPC handshake");
  parse_option_net_builtin("force-dh", no_argument, 0, 230, LONGOPT_TCP_SET,
                           "Force using DH for all outbound RPC connections");
  parse_option_net_builtin(
      "max-accept-rate", required_argument, 0, 249, LONGOPT_TCP_SET,
      "max number of connections per second that is allowed to accept");
  parse_option_net_builtin(
      "max-dh-accept-rate", required_argument, 0, 250, LONGOPT_TCP_SET,
      "max number of DH connections per second that is allowed to accept");
  parse_option_net_builtin("nat-info", required_argument, 0, 372,
                           LONGOPT_NET_SET,
                           "<local-addr>:<global-addr>\tsets network address "
                           "translation for RPC protocol handshake");
  parse_option_net_builtin("address", required_argument, 0, 373,
                           LONGOPT_NET_SET,
                           "tries to bind socket only to specified address");
}

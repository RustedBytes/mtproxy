...
#define TL_PARSE_FUN_EX(tname, fname, dname, qtype, ...)                       \
  static struct tl_act_extra *fname(struct tl_in_state *tlio_in,               \
                                    ##__VA_ARGS__) {                           \
    struct tl_act_extra *extra =                                               \
        tl_act_extra_init(stats_buff, sizeof(tname), dname);                   \
    tname *e __attribute__((unused));                                          \
    e = (void *)extra->extra;                                                  \
    extra->type = qtype;                                                       \
    extra->subclass = -1;

#define TL_PARSE_FUN(name, ...)                                                \
  TL_PARSE_FUN_EX(struct tl_##name, tl_##name, tl_do_##name, __VA_ARGS__)
#define TL_PARSE_FUN_GET(name, ...)                                            \
  TL_PARSE_FUN_EX(struct tl_##name, tl_##name, tl_do_##name,                   \
                  QUERY_ALLOW_REPLICA_GET | QUERY_ALLOW_REPLICA_SET,           \
                  ##__VA_ARGS__)
#define TL_PARSE_FUN_GET_ONLY(name, ...)                                       \
  TL_PARSE_FUN_EX(struct tl_##name, tl_##name, tl_do_##name,                   \
                  QUERY_ALLOW_REPLICA_GET, ##__VA_ARGS__)
#define TL_PARSE_FUN_SET(name, ...)                                            \
  TL_PARSE_FUN_EX(struct tl_##name, tl_##name, tl_do_##name,                   \
                  QUERY_ALLOW_REPLICA_SET, ##__VA_ARGS__)
...
/* ${engine}-interface-structures.h must contain #pragma pack(push,4) for use
 * TL_DEFAULT_PARSE_FUN macro */
#define TL_DEFAULT_PARSE_FUN(name, qtype)                                      \
  TL_PARSE_FUN(name, qtype)                                                    \
  if (tlf_check(tlio_in, sizeof(*e)) < 0) {                                    \
...
#define TL_DEFAULT_PARSE_FUN_GET_ONLY(name)                                    \
  TL_DEFAULT_PARSE_FUN(name, QUERY_ALLOW_REPLICA_GET)
#define TL_DEFAULT_PARSE_FUN_GET(name)                                         \
  TL_DEFAULT_PARSE_FUN(name, QUERY_ALLOW_REPLICA_GET | QUERY_ALLOW_REPLICA_SET)
#define TL_DEFAULT_PARSE_FUN_SET(name)                                         \
  TL_DEFAULT_PARSE_FUN(name, QUERY_ALLOW_REPLICA_SET)
...
#define TL_DO_FUN(name)                                                        \
  TL_DO_FUN_EX(struct tl_##name __attribute__((unused)), tl_do_##name);
#define TL_DO_FUN_DECL(name)                                                   \
  TL_DO_FUN_DECL_EX(struct tl_##name __attribute__((unused)), tl_do_##name);
...
#define TL_DO_PUBLIC_FUN(name)                                                 \
  TL_DO_PUBLIC_FUN_EX(struct tl_##name __attribute__((unused)), tl_do_##name);
#define TL_DO_PUBLIC_FUN_DECL(name)                                            \
  TL_DO_PUBLIC_FUN_DECL_EX(struct tl_##name __attribute__((unused)),           \
                           tl_do_##name);

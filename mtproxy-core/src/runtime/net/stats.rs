...
mod tests {
    ...
    fn recent_idle_uses_ratio_when_quotient_positive() {
        assert_f64_eq(recent_idle_percent(1.5, 3.0), 50.0);
    }

    #[test]
    fn recent_idle_falls_back_to_time_when_quotient_non_positive() {
        assert_f64_eq(recent_idle_percent(7.25, 0.0), 7.25);
        assert_f64_eq(recent_idle_percent(7.25, -1.0), 7.25);
    }

    #[test]
    fn average_idle_requires_positive_uptime() {
        assert_f64_eq(average_idle_percent(15.0, 30), 50.0);
        assert_f64_eq(average_idle_percent(15.0, 0), 0.0);
        assert_f64_eq(average_idle_percent(15.0, -1), 0.0);
    }
}

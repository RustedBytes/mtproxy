...
static int32_t c_pick_size_index (const int32_t *sizes, int32_t n, int32_t size_hint) {
  if (!sizes || n <= 0) {
    return -1;
  }
  int32_t si = n - 1;
...
int main (void) {
  ...
  assert ((boundary.net_events_contract_ops & expected_events_contract_ops) == expected_events_contract_ops);
  assert ((boundary.net_events_implemented_ops & expected_events_contract_ops) == expected_events_contract_ops);

  const uint32_t expected_timers_contract_ops = MTPROXY_FFI_NET_TIMERS_OP_WAIT_MSEC;
  assert ((boundary.net_timers_contract_ops & expected_timers_contract_ops) == expected_timers_contract_ops);
  assert ((boundary.net_timers_implemented_ops & expected_timers_contract_ops) == expected_timers_contract_ops);

  const uint32_t expected_msg_buffers_contract_ops = MTPROXY_FFI_NET_MSGBUFFERS_OP_PICK_SIZE_INDEX;
  assert ((boundary.net_msg_buffers_contract_ops & expected_msg_buffers_contract_ops) == expected_msg_buffers_contract_ops);
  assert ((boundary.net_msg_buffers_implemented_ops & expected_msg_buffers_contract_ops) == expected_msg_buffers_contract_ops);

  assert ((boundary.net_events_implemented_ops & ~boundary.net_events_contract_ops) == 0);
  assert ((boundary.net_timers_implemented_ops & ~boundary.net_timers_contract_ops) == 0);
  assert ((boundary.net_msg_buffers_implemented_ops & ~boundary.net_msg_buffers_contract_ops) == 0);

  const int conv_cases[] = {
    0,
    EVT_READ,
    EVT_WRITE,
    EVT_SPEC,
  ...
  static const struct timer_case timer_cases[] = {
    {10.010, 10.000},
    {10.000, 10.000},
    {9.900, 10.000},
    {1000.250, 1000.125}
  ...
  for (size_t i = 0; i < sizeof (timer_cases) / sizeof (timer_cases[0]); i++) {
    int32_t got = mtproxy_ffi_net_timers_wait_msec (timer_cases[i].wakeup_time, timer_cases[i].now);
    int32_t expected = c_timers_wait_msec (timer_cases[i].wakeup_time, timer_cases[i].now);
    assert (got == expected);
  }

  static const int32_t buffer_sizes[] = {48, 512, 2048, 16384, 262144};
  static const int32_t hints[] = {-1, 0, 48, 49, 512, 513, 3000, 200000, 300000};
  for (size_t i = 0; i < sizeof (hints) / sizeof (hints[0]); i++) {
    int32_t got = mtproxy_ffi_msg_buffers_pick_size_index (
      buffer_sizes,
      (int32_t) (sizeof (buffer_sizes) / sizeof (buffer_sizes[0])),
      hints[i]
    );

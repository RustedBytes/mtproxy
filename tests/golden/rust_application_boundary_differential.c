...
static int c_engine_rpc_result_header_len(int flags) { return flags ? 8 : 0; }

static int c_mtproto_ext_conn_hash(int in_fd, long long in_conn_id,
                                   int hash_shift) {
  unsigned long long h =
      (unsigned long long)in_fd * 11400714819323198485ULL +
...
int main(void) {
  ...
  assert((boundary.engine_rpc_contract_ops & expected_engine_contract) ==
         expected_engine_contract);
  assert((boundary.engine_rpc_implemented_ops & expected_engine_contract) ==
         expected_engine_contract);
  assert((boundary.mtproto_proxy_contract_ops & expected_mtproto_contract) ==
         expected_mtproto_contract);
  assert((boundary.mtproto_proxy_implemented_ops & expected_mtproto_contract) ==
         expected_mtproto_contract);

  assert((boundary.engine_rpc_implemented_ops &
          ~boundary.engine_rpc_contract_ops) == 0);
  assert((boundary.mtproto_proxy_implemented_ops &
          ~boundary.mtproto_proxy_contract_ops) == 0);

  const int flags_cases[] = {0, 1, 0x12345678, (int)0xffffffffu,
                             (int)0x80000000u};
  for (size_t i = 0; i < sizeof(flags_cases) / sizeof(flags_cases[0]); i++) {
    ...
    int got_header_len =
        mtproxy_ffi_engine_rpc_result_header_len(flags_cases[i]);
  ...
  const struct {
    ...
    int hash_shift;
  } hash_cases[] = {{42, 0x123456789abcdef0ll, 20},
                    {-1, -17, 20},
                    {0, 0, 20},
                    {65535, 0x7ffffffffffffffell, 20}};
  for (size_t i = 0; i < sizeof(hash_cases) / sizeof(hash_cases[0]); i++) {
    int got = mtproxy_ffi_mtproto_ext_conn_hash(hash_cases[i].in_fd,
                                                hash_cases[i].in_conn_id,
                                                hash_cases[i].hash_shift);
    int expected =
        c_mtproto_ext_conn_hash(hash_cases[i].in_fd, hash_cases[i].in_conn_id,
                                hash_cases[i].hash_shift);
    assert(got == expected);
  }
  assert(mtproxy_ffi_mtproto_ext_conn_hash(1, 2, 0) == -1);

  const int generation_cases[] = {0, 1, 0x12345678, (int)0xffffffffu,
                                  (int)0x80000000u};
  for (size_t i = 0; i < sizeof(generation_cases) / sizeof(generation_cases[0]);
       i++) {

...
int main(void) {
  ...
  assert((boundary.mpq_contract_ops & expected_mpq_contract_ops) == expected_mpq_contract_ops);
  const uint32_t expected_mpq_implemented_ops =
    MTPROXY_FFI_MPQ_OP_PUSH |
    MTPROXY_FFI_MPQ_OP_POP |
    MTPROXY_FFI_MPQ_OP_IS_EMPTY;
  assert((boundary.mpq_implemented_ops & expected_mpq_implemented_ops) == expected_mpq_implemented_ops);
  ...
  assert((boundary.jobs_contract_ops & expected_jobs_contract_ops) == expected_jobs_contract_ops);
  const uint32_t expected_jobs_implemented_ops =
    MTPROXY_FFI_JOBS_OP_CREATE_ASYNC_JOB |
    MTPROXY_FFI_JOBS_OP_JOB_SIGNAL |
    MTPROXY_FFI_JOBS_OP_JOB_INCREF |
    MTPROXY_FFI_JOBS_OP_JOB_DECREF;
  assert((boundary.jobs_implemented_ops & expected_jobs_implemented_ops) == expected_jobs_implemented_ops);

...
static int c_is_good_rpc_dh_bin(const unsigned char *data,
                                const unsigned char *prime_prefix) {
  int i;
  int ok = 0;
  for (i = 0; i < 8; i++) {
    if (data[i]) {
...
static int c_aes_create_keys(mtproxy_ffi_aes_key_data_t *R, int am_client,
                             ...
                             unsigned client_ip, unsigned short client_port,
                             const unsigned char client_ipv6[16],
                             const unsigned char *secret, int secret_len,
                             const unsigned char *temp_key, int temp_key_len) {
  unsigned char str[16 + 16 + 4 + 4 + 2 + 6 + 4 + 2 + MAX_PWD_LEN + 16 + 16 +
                    4 + 16 * 2 + 256];
  int i, str_len;
...
int main(void) {
  ...
  assert((boundary.net_crypto_aes_contract_ops & expected_aes_ops) ==
         expected_aes_ops);
  assert((boundary.net_crypto_aes_implemented_ops & expected_aes_ops) ==
         expected_aes_ops);
  assert((boundary.net_crypto_dh_contract_ops & expected_dh_ops) ==
         expected_dh_ops);
  assert((boundary.net_crypto_dh_implemented_ops & expected_dh_ops) ==
         expected_dh_ops);
  assert((boundary.aesni_contract_ops & expected_aesni_ops) ==
         expected_aesni_ops);
  assert((boundary.aesni_implemented_ops & expected_aesni_ops) ==
         expected_aesni_ops);

  assert((boundary.net_crypto_aes_implemented_ops &
          ~boundary.net_crypto_aes_contract_ops) == 0);
  assert((boundary.net_crypto_dh_implemented_ops &
          ~boundary.net_crypto_dh_contract_ops) == 0);
  assert((boundary.aesni_implemented_ops & ~boundary.aesni_contract_ops) == 0);

  const unsigned char prime_prefix[8] = {0x89, 0x52, 0x13, 0x1b,
                                         0x1e, 0x3a, 0x69, 0xba};
  ...
  for (size_t i = 0; i < sizeof(dh_cases) / sizeof(dh_cases[0]); i++) {
    ...
    int got = mtproxy_ffi_crypto_dh_is_good_rpc_dh_bin(dh_cases[i], 256,
                                                       prime_prefix, 8);
    assert(got == expected);
  }
  ...
  const unsigned char nonce_server[16] = {0x10, 0x11, 0x12, 0x13, 0x14, 0x15,
                                          0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b,
                                          0x1c, 0x1d, 0x1e, 0x1f};
  const unsigned char nonce_client[16] = {0x20, 0x21, 0x22, 0x23, 0x24, 0x25,
                                          0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b,
                                          0x2c, 0x2d, 0x2e, 0x2f};
  const unsigned char server_ipv6[16] = {0x30, 0x31, 0x32, 0x33, 0x34, 0x35,
                                         0x36, 0x37, 0x38, 0x39, 0x3a, 0x3b,
                                         0x3c, 0x3d, 0x3e, 0x3f};
  const unsigned char client_ipv6[16] = {0x40, 0x41, 0x42, 0x43, 0x44, 0x45,
  ...
  rc = mtproxy_ffi_crypto_aes_create_keys(
      &rust_keys, 1, nonce_server, nonce_client, 1700000000, 0x0a000001u, 443,
      server_ipv6, 0x0a000002u, 32000, client_ipv6, secret, 32, temp_key, 64);
  assert(rc == 1);
  assert(c_aes_create_keys(&c_keys, 1, nonce_server, nonce_client, 1700000000,
                           0x0a000001u, 443, server_ipv6, 0x0a000002u, 32000,
                           client_ipv6, secret, 32, temp_key, 64) == 1);
  assert(memcmp(&rust_keys, &c_keys, sizeof(rust_keys)) == 0);

  rc = mtproxy_ffi_crypto_aes_create_keys(
      &rust_keys, 0, nonce_server, nonce_client, 1700000000, 0, 443,
      server_ipv6, 0, 32000, client_ipv6, secret, 32, temp_key, 64);
  assert(rc == 1);
  assert(c_aes_create_keys(&c_keys, 0, nonce_server, nonce_client, 1700000000,
                           0, 443, server_ipv6, 0, 32000, client_ipv6, secret,
                           32, temp_key, 64) == 1);
  assert(memcmp(&rust_keys, &c_keys, sizeof(rust_keys)) == 0);

...
int main(void) {
  ...
  for (size_t s = 0; s < sizeof(seeds) / sizeof(seeds[0]); s++) {
    for (size_t len = 0; len <= sizeof(data); len++) {
      ...
      c_split =
          crc32_partial_generic(data + split, (long)(len - split), c_split);
      ...
      if (c_split != r_split) {
        fprintf(stderr,
                "crc32 split mismatch seed=%08x len=%zu c=%08x rust=%08x\n",
  ...
  unsigned known =
      mtproxy_ffi_crc32_partial(k_known, 9, 0xffffffffu) ^ 0xffffffffu;

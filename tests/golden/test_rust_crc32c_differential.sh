...
if ! command -v cargo >/dev/null 2>&1; then
  log_skip "Rust CRC32C differential test skipped: cargo is not available"
  exit 0
fi
...
BIN="$OUT_DIR/rust_crc32c_differential"

log_info "Golden: compiling C-vs-Rust CRC32C differential test"
gcc -std=gnu11 -O2 \
...
"$BIN" > "$OUT_DIR/rust_crc32c_differential.log" 2>&1

assert_contains "$OUT_DIR/rust_crc32c_differential.log" "rust_crc32c_differential: ok"
log_pass "Rust CRC32C differential vectors match C implementation"

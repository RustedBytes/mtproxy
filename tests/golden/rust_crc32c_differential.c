...
int main(void) {
  ...
  for (size_t s = 0; s < sizeof(seeds) / sizeof(seeds[0]); s++) {
    for (size_t len = 0; len <= sizeof(data); len++) {
      ...
      size_t split = len / 5;
      unsigned c_split = crc32c_partial_four_tables(data, (long) split, seeds[s]);
      c_split = crc32c_partial_four_tables(data + split, (long) (len - split), c_split);
      ...
      if (c_split != r_split) {
        fprintf(stderr, "crc32c split mismatch seed=%08x len=%zu c=%08x rust=%08x\n", seeds[s], len, c_split, r_split);
        return 1;
      }
    }
  }
  ...
  unsigned known_crc = mtproxy_ffi_crc32c_partial(known, 9, 0xffffffffu) ^ 0xffffffffu;
  if (known_crc != 0xe3069283u) {
    fprintf(stderr, "known vector mismatch got=%08x expected=%08x\n", known_crc, 0xe3069283u);
    return 1;
  }

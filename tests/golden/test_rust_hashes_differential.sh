...
if ! command -v cargo >/dev/null 2>&1; then
  log_skip "Rust hashes differential test skipped: cargo is not available"
  exit 0
fi
...
BIN="$OUT_DIR/rust_hashes_differential"

log_info "Golden: compiling Rust hash differential test"
gcc -std=gnu11 -O2 \

...
if ! command -v cargo >/dev/null 2>&1; then
  log_skip "Rust CRC32 differential test skipped: cargo is not available"
  exit 0
fi
...
BIN="$OUT_DIR/rust_crc32_differential"

log_info "Golden: compiling C-vs-Rust CRC32 differential test"
gcc -std=gnu11 -O2 \
  -I"$ROOT_DIR" -I"$ROOT_DIR/common" \
  "$ROOT_DIR/tests/golden/rust_crc32_differential.c" \
  "$ROOT_DIR/objs/lib/libkdb.a" \
  "$ROOT_DIR/target/debug/libmtproxy_ffi.a" \
  -lm -lrt -lcrypto -lz -lpthread -ldl \
  -o "$BIN"

log_info "Golden: executing C-vs-Rust CRC32 differential test"
"$BIN" > "$OUT_DIR/rust_crc32_differential.log" 2>&1

assert_contains "$OUT_DIR/rust_crc32_differential.log" "rust_crc32_differential: ok"
log_pass "Rust CRC32 differential vectors match C implementation"
